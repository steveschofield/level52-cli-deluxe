#!/usr/bin/env python3
"""
Install ProjectDiscovery httpx (scanner) into tools/.bin/httpx.

This avoids conflicts with the Python `httpx` package, which also installs an `httpx` CLI.
"""
...
def _pick_asset(release: dict, suffix: str) -> tuple[str, str]:
    for asset in release.get("assets", []):
        ...
        if name.endswith(".zip") and suffix in name and "httpx" in name.lower():
            return name, url
...
def main() -> int:
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        members = [m for m in zf.namelist() if m.endswith("/httpx") or m == "httpx"]
        if not members:
            raise SystemExit("Downloaded archive does not contain an 'httpx' binary")
        member = members[0]
    ...
    print(f"Installed ProjectDiscovery httpx to {out_path}", file=sys.stderr)
    print("Optional: set GUARDIAN_HTTPX_BIN to override.", file=sys.stderr)
    return 0
